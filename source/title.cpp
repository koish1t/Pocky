...
bool Title::load(u64 _id, FS_MediaType _media)
{
	...
	backupPath = u8tou16("/3ds/Pocky/mods/") + u8tou16(unique) + shortDescription;
	...
	if (!directoryExist(getArchiveSDMC(), backupPath))
	{
		...
		if (R_FAILED(res))
		{
			createError(res, "Failed to create mods directory.");
		}
	}
...
void getTitle(Title &dst, int i)
{
	...
	if (i < getTitlesCount())
	{
		dst = mode == MODE_MOD ? titleSaves.at(i) : titleExtdatas.at(i);
	}	
}
...
{
	const Mode_t mode = getMode();
	return mode == MODE_MOD ? titleSaves.size() : titleExtdatas.size();
}

size_t getTextureId(int i)
{
	const Mode_t mode = getMode();
	return mode == MODE_MOD ? titleSaves.at(i).getTextureId() : titleExtdatas.at(i).getTextureId();
}
